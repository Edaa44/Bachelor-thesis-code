...
class VarValueRenaming:
    ...
    def apply_to_task(self, task):
        ...
        task.global_constraint = self.translate_global_constraint(
    ...
    def translate_global_constraint(self, constraint):
        ...
        print(
            "Simplifyed global constraint to new variable ordering %s"
            % [new_constraint]
        )
        ...
        if new_constraint[1] == always_true:
            assert False, (
                "Regular Task without global constraint"
            )  # somehow the created axiom is not simplified to always true and we never end up here
