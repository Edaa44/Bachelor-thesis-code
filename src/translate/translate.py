...
def strips_to_sas_dictionary(
    groups, num_axioms, num_axiom_map, num_fluents, assert_partial, include_numeric=True
):
    dictionary = {}
    numeric_dictionary = {}
    ...
    if include_numeric:
        ...
        for axiom_effect in redundant_axioms:
            numeric_dictionary[axiom_effect] = numeric_dictionary[
                num_axiom_map[axiom_effect].effect
            ]
    ...
    for fact in conditions:
        if isinstance(fact, pddl.FunctionComparison) or isinstance(
            ...
            if fact not in dictionary:
                ...
                for (
                    part
                ) in fact.parts:  # TODO: remove this assertion check when not debugging
...
def translate_strips_conditions(
    ...
    if (
        translate_strips_conditions_aux(
            conditions, mutex_dict, mutex_ranges, numeric_dictionary, comp_axioms, True
        )
        is None
    ):
        return None
    ...
    return translate_strips_conditions_aux(
        conditions, dictionary, ranges, numeric_dictionary, comp_axioms
    )
...
def negate_and_translate_condition(
    condition, dictionary, ranges, numeric_dict, mutex_dict, mutex_ranges, comp_axioms
):
...
def build_sas_operator(
    ...
    for numvar in ass_effects_by_variable:
        ...
        #    respective arrays
        for (ass_op, post_var), eff_conditions in ass_effects_by_variable[
            numvar
        ].items():
    ...
    if (
        not pre_post
    ):  # build only operators with at least one regular effect (not instrumentation)
        irrelevant = True
...
def translate_strips_axiom(
    axiom, dictionary, ranges, num_dict, mutex_dict, mutex_ranges, comp_ax
):
    conditions = translate_strips_conditions(
...
def translate_strips_axioms(
    axioms, strips_to_sas, ranges, num_dict, mutex_dict, mutex_ranges, comp_ax
):
    result = []
    for axiom in axioms:
        sas_axioms = translate_strips_axiom(
            axiom, strips_to_sas, ranges, num_dict, mutex_dict, mutex_ranges, comp_ax
        )
...
def add_key_to_comp_axioms(
    ...
    for axiom in comparison_axioms[1]:
        value_list = []
        assert axiom.effect == len(translation_key), (
            "current effect %s != next variable %s"
            % (axiom.effect, len(translation_key))
        )
...
def translate_task(
    ...
    assert (
        len(global_constraint_dict_list) == 1
    )  # the key is the axiom fluent, the value (0) the evaluation to true
    ...
    variables = sas_tasks.SASVariables(
        ranges, axiom_layers, translation_key, num_axiom_layer
    )
    ...
    numeric_variables = sas_tasks.SASNumericVariables(
        num_variables, num_axiom_layers, num_var_types
    )
    ...
    if metric[1] == -1:
        ...
        assert metric[1] in numeric_strips_to_sas, (
            "Metric fluent %s missing in strips_to_sas_dict" % metric[1]
        )
    ...
    return sas_tasks.SASTask(
        ...
        comparison_axioms[1],
        sas_num_axioms,
        list(global_constraint_dict_list[0].items())[0],
        sas_metric,
        init_constant_predicates,
        init_constant_numerics,
    )
...
def trivial_task(solvable):
    ...
    num_variables = sas_tasks.SASNumericVariables(
        ["total-cost"], [-1], ["I"]
...
def pddl_to_sas(task):
    ...
    if DEBUG:
        print(
            "List of %d constant predicates from initial state"
    ...
    if DEBUG:
        print(
            "List of %d constant numeric predicates from initial state"
    ...
    with timers.timing("Building STRIPS to SAS dictionary"):
        ranges, strips_to_sas, num_count, numeric_strips_to_sas = (
