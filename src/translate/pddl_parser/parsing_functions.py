...
def parse_function(alist, type_name):
    ...
    pddl.Task.FUNCTION_SYMBOLS[name] = (
        type_name  # add function name to function symbol dictionary
    )
...
def is_function_comparison(alist):
    ...
    if isinstance(symbol, list):
        ...
        symbol = symbol[0]
    if (
        pddl.Task.FUNCTION_SYMBOLS.get(symbol, "object") == "number"
        or symbol.replace(".", "").isdigit()
    ):
    ...
    if isinstance(tmp_effect, pddl.ConjunctiveEffect):
        ...
        if isinstance(tmp_effect, pddl.UniversalEffect):
            ...
            if isinstance(tmp_effect.effect, pddl.ConditionalEffect):
                ...
                assert isinstance(
                    tmp_effect.effect.effect, pddl.SimpleEffect
    ...
    if effect_list:
        try:
            ...
            if cost is None and eff:
                if DEBUG:
                    print(
                        "adding artificial effect (increase total-cost 1) to %s" % name
                    )
...
def parse_global_constraint(
    alist, type_dict, predicate_dict
):  # global constraints are parsed as special Axioms
    if DEBUG:
    ...
    for entry in iterator:
        if entry[0] == ":metric":
            if entry[1] == "minimize":
                ...
                assert entry[1] == "maximize", (
                    "Unknown metric, 'minimize' or 'maximize' expected."
                )
                metric_symbol = ">"
            #            try:
