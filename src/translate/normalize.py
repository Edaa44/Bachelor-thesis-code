...
def all_conditions(task):
    for action in task.actions:
        ...
        for effect in (
            action.effects
        ):  # NFD: also includes numeric cost effect as regular increas effect
...
def remove_arithmetic_expressions(task):
    def recurse(condition):
        if isinstance(condition, pddl.FunctionComparison):
            parts = [
                task.function_administrator.get_derived_function(exp)
    ...
    if metric_expression is None:  # unit cost -> no metric specified in task
        assert opt_dir == "<", (
            "No metric expression specified in task, but 'maximize' found"
        )
        metric_expression = -1
    else:
...
def normalize(task):
    ...
    verify_and_fix_arithmetic_expressions(
        task
    )  # fix errors from invalid PDDL specifications
    remove_arithmetic_expressions(
        task
    )  # replace complicated expressions with their derived variable
...
def verify_axiom_predicates(task):
    ...
    for action in task.actions:
        for effect in action.effects:
            if (
                isinstance(effect.peffect, pddl.Literal)
                and effect.peffect.predicate in axiom_names
            ):
                raise SystemExit(
