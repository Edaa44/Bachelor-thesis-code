...
class Condition:
    def __init__(self, parts):
        ...
        if DEBUG:
            print(
                "Initialized %s Condition with the following %d parts:"
                % (self.__class__, len(parts))
            )
        if DEBUG:
            for p in parts:
                print(p)
...
class FunctionComparison(
    Condition
):  # NFD class yielding the truth value of a function comparison
    negated = False
    ...
    def __init__(self, comparator, parts, compare_to_zero=False):
        ...
        if compare_to_zero:
            self.parts = (
                f_expression.Difference(parts),
                f_expression.NumericConstant(0.0),
            )
        else:
            self.parts = tuple(parts)
    ...
    def uniquify_variables(self, type_map, renamings={}):
        return self.__class__(
            self.comparator, [part.rename_variables(renamings) for part in self.parts]
        )
    ...
    def instantiate(
        ...
        instantiated_parts = [
            part.instantiate(
                var_mapping, fluent_functions, init_function_vals, task, new_axiom
            )
            for part in self.parts
        ]
