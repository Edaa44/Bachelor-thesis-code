...
class ConjunctiveEffect:
    ...
    def normalize(self):
        ...
        return ConjunctiveEffect(
            new_effects
        )  # creating a new effect flattens it automatically
    ...
    def extract_cost(self):
        ...
        for part in self.effects:
            if (
                isinstance(part.effect, f_expression.FunctionAssignment)
                and part.effect.is_cost_assignment()
            ):
                assert cost_effect == None, "multiple values assigned to cost effect"
                cost_effect = part
            new_effects.append(
                part
            )  # even if the effect is a cost effect it is just a "regular" numeric effect
