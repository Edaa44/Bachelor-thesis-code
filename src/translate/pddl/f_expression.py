    ...
    def compile_objectfunctions_aux(self, used_variables, recurse_object_terms=True):
        ...
        for part in self.parts:
            typed, parts, new_part = part.compile_objectfunctions_aux(
                used_variables, recurse_object_terms
            )
            typed_vars += typed
        ...
        if fluent_functions is not None:
            if pne not in fluent_functions and not pne.symbol.startswith("derived!"):
                if pne not in init_function_vals:
                    raise ValueError(
                        f"Cannot instantiate non-fluent PNE: no initial value given {pne}"
                    )
                ...
                new_axiom = new_axiom.instantiate(
...
class FunctionAssignment:
    ...
    def rename_variables(
        self, renamings
    ):  # NFD: FunctionAssignments are no longer restricted to "total-cost"
        return self.__class__(
            self.fluent.rename_variables(renamings),
    ...
    def instantiate(
        ...
        fluent = self.fluent.instantiate(
            var_mapping, fluent_functions, init_function_vals, task, new_axioms
        )
        try:
    ...
    def instantiate_cost(self, var_mapping, fluent_functions, init_function_vals, task):
        assert self.is_cost_assignment(), (
            "trying to instantiate cost from non cost function assignment"
        )
        try:
