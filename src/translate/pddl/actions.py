        ...
        if effect_list:
            ...
            if task.metric[1] != -1:
                if self.cost is None:
                    ...
                    cost = self.cost.effect.instantiate_cost(
                        var_mapping, fluent_functions, init_function_vals, task
                    )
...
class PropositionalAction:
    def __init__(self, name, precondition, effects, cost):
        ...
        self.cost = (
            cost  # deprecated float, cost effects are regular assignment effects now
        )
