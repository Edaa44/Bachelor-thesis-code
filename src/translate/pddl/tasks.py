    ...
    def add_global_constraints(
        ...
        for axiom in self.axioms:
            if axiom.is_global_constraint:
                ...
                universe = conditions.UniversalCondition(
    ...
    def get_derived_function(self, exp):
        ...
        if isinstance(exp, f_expression.PrimitiveNumericExpression):
            ...
            if key not in self.functions:
                ...
                subexp = f_expression.PrimitiveNumericExpression(
                    subexp.symbol, default_args, "S"
                )
                self.functions[key] = axioms.NumericAxiom(
                    symbol, default_args, exp.op, [subexp]
                )
        else:
            if DEBUG:
            ...
            if key not in self.functions:
                ...
                for df in keylist[1:]:
                    pnelist.append(
                        f_expression.PrimitiveNumericExpression(
                            df.symbol,
                            default_args[argindex : argindex + len(df.args)],
                            "D",
                        )
                    )
                    if DEBUG:
                ...
                assert pnelist
                self.functions[key] = axioms.NumericAxiom(
                    symbol, tuple(default_args), exp.op, pnelist
                )
