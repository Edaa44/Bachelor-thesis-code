...
class SASVariables:
    def __init__(self, ranges: List[int], axiom_layers: List[int], value_names: List[List[str]], comp_axiom_layer: int):
        self.ranges: List[int] = ranges
        self.axiom_layers: List[int] = axiom_layers
        self.value_names: List[List[str]] = value_names
        self.comp_axiom_layer: int = comp_axiom_layer  # id of the comparison axiom layer -> needed to validate that logic axioms can only have 2 values
    ...
    def validate_fact(self, fact: Tuple[int, int]):
        ...
        assert 0 <= value < self.ranges[var], "value= %d, self.ranges[var]=%d" % (
            value,
            self.ranges[var],
        )

    def validate_condition(self, condition: List[Tuple[int, int]]):
    ...
    def dump(self):
        for var, (rang, names, axiom_layer) in enumerate(
            ...
            print(
                "v%d in {%s}%s"
                % (var, list("%d:%s" % t for t in zip(range(rang), names)), axiom_str)
            )
...
class SASNumericVariables:
    def __init__(self, variable_names: List[int], axiom_layers: List[int], types: List[str]):
        self.variable_names: List[int] = variable_names
        self.axiom_layers: List[int] = axiom_layers
    ...
    def simple_output(self, stream):
        print(len(self.variable_names), file=stream)
        print("begin_numeric_variables", file=stream)
        v = 0
        for nv in self.variable_names:
            print(f"{nv.symbol}({', '.join(map(str, nv.args))})", file=stream)
            v += 1
        print("end_numeric_variables", file=stream)


class SASMutexGroup:
    def __init__(self, facts: List[Tuple[int, int]]):
        self.facts: List[Tuple[int, int]] = sorted(facts)
...
class SASInit:
    def __init__(self, values: List[int], numeric_values: List[float]):
        self.values: List[int] = values
        self.num_values: List[float] = numeric_values
    ...
    def validate(self, variables: SASVariables):
        ...
        assert len(self.values) == len(variables.ranges), (
            "length of values= %s length of varranges= %s"
            % (len(self.values), len(variables.ranges))
        )
...
class SASOperator:
    def __init__(self, name: str, prevail: List[Tuple[int, int]], pre_post: List[tuple], assign_effect: List[Tuple[int, str, int, list]], cost: float):
        self.name: str = name
        self.prevail: List[Tuple[int, int]] = sorted(prevail)
        self.pre_post: List[tuple] = self._canonical_pre_post(pre_post)
        self.assign_effects: List[Tuple[int, str, int, list]] = sorted(assign_effect)
        self.cost: float = cost

    def _canonical_pre_post(self, pre_post: List[tuple]) -> List[tuple]:
    ...
    def dump(self):
        ...
        for var, ass_op, ass_var, cond in self.assign_effects:
            if cond:
                ...
                cond_str = ""
            print(
                "  nv%d: %s nv%d%s" % (var, ass_op, ass_var, cond_str)
            )  # ass_op in {+-*/=}
        print(
            "Cost:"
    ...
    def get_applicability_conditions(self) -> List[Tuple[int, int]]:
...
class SASAxiom:
    def __init__(self, condition: List[Tuple[int, int]], effect: Tuple[int, int]):
        self.condition: List[Tuple[int, int]] = sorted(condition)
        self.effect: Tuple[int, int] = effect
        assert self.effect[1] in (0, 1)

        for _, val in condition:
            assert val >= 0, condition

    def validate(self, variables: SASVariables, init: SASInit):
        """Validate the axiom.
...
class SASCompareAxiom:
    ...
    def __init__(self, comp: str, parts: List[int], effect: int):
        self.comp: str = comp
    ...
    def pretty_print(self, numeric_variables: SASNumericVariables):
        print(
            "CAX %s '%s' '%s'"
            % (
                self.comp,
...
class SASNumericAxiom:  # takes an arithmetic operator ("+", "-", "*", "/") and 3 numeric fluents where the 3rd represents the computation of the first two.
    def __init__(self, op: str, parts: List[int], effect: int):
        self.op: str = op
        self.parts: List[int] = parts
        self.effect: int = effect
...
class SASTask:
    ...
    def __init__(
        ...
        comp_axioms: List[SASCompareAxiom],
        numeric_axioms: List[SASNumericAxiom],
        global_constraint: Tuple[int, int],
        metric: Tuple[str, int],
        init_constant_predicates: set,
        init_constant_numerics: set,
    ):
